...
export default function FarmerDashboard() {
  ...
  const activeProducts = products.filter(p => p.is_active && p.quantity_kg > 0);
  const recentProducts = products.slice(0, 3); // Show top 3 most recent

  const farmerName = profile?.full_name || 'Farmer';
  const farmerLocation = profile?.location || '';
    ...
    <FarmerLayout>
      <div className="flex flex-col gap-6">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Good morning, {farmerName.split(' ')[0]} 👋</h1>
            <div className="flex items-center gap-3 mt-1">
              <span className="text-gray-500 text-sm">🌿 FarmDirect Farmer</span>
              {farmerLocation && (
                <>
                  <span className="text-gray-400">·</span>
                  <span className="text-gray-500 text-sm">📍 {farmerLocation}</span>
                </>
              )}
              {isVerified && (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard label="Total Sales" value="₹0" icon="₹" color="green" />
          <StatCard label="Active Products" value={String(activeProducts.length)} icon="🌿" color="blue" />
          <StatCard label="Pending Orders" value="0" icon="📦" color="amber" />
          <StatCard label="Rating" value="—" icon="⭐" color="purple" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            ...
            {loading ? (
              ...
              <Card className="p-8 text-center">
                ...
                <p className="text-gray-500 mb-4">No products yet. Add your first produce!</p>
                <Btn onClick={() => navigate('/farmer/add-product')}>+ Add Product</Btn>
              </Card>
            ) : (
              <div className="flex flex-col gap-3">
                {recentProducts.map(p => {
                  const status = p.quantity_kg <= 0 || !p.is_active ? 'sold' : p.quantity_kg < 100 ? 'low' : 'active';
                  return (
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <Badge variant={status === 'active' ? 'active' : status === 'low' ? 'low' : 'sold'}>
                            {status === 'active' ? 'Active' : status === 'low' ? 'Low Stock' : 'Sold Out'}
                          </Badge>
                        </div>
                        <p className="text-sm text-gray-500">{p.quantity_kg} {p.unit || 'kg'} · ₹{p.price_per_kg}/kg · {p.grade || p.category}</p>
                      </div>
          ...
          {/* Insights — informational features for future phases */}
          <div className="flex flex-col gap-4">
            ...
            <Card className="p-5 bg-[#2E7D32] border-0">
              ...
              <p className="text-green-200 text-sm mb-3">Orders coming soon.</p>
