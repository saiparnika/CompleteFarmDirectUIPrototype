...
export default function AddProduct() {
  ...
  const [form, setForm] = useState({
    name: '', category: 'Vegetables', qty: '', unit: 'kg',
    price: '', quality: 'Grade A', harvest: '', location: '',
  });
  ...
  const handlePublish = async () => {
    ...
    if (!form.qty.trim()) { setError('Quantity is required.'); return; }
    ...
    const { error: createError } = await createProduct(
      {
        ...
        grade: form.quality,
        unit: form.unit,
        harvest_date: form.harvest || null,
        ...
        <div className="flex items-center justify-center min-h-[60vh]">
          <Card className="p-10 text-center max-w-md w-full">
            ...
            <div className="flex flex-col gap-3">
              ...
              <Btn full variant="outline" onClick={handleReset}>Add Another Product</Btn>
            </div>
          </Card>
        </div>
      </FarmerLayout>
        ...
        <Card className="p-6">
          ...
          <div className="mt-5">
            ...
            <div
              ...
              className="border-2 border-dashed border-gray-200 rounded-xl p-8 text-center hover:border-[#2E7D32]/50 transition cursor-pointer"
            >
              {imagePreview ? (
          ...
          <div className="mt-5 bg-amber-50 border border-amber-100 rounded-xl p-4 flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-700 mt-0.5">Current market suggests ₹27–₹30/kg for Tomato. Your price of ₹{form.price || '0'}/kg is within range.</p>
            </div>
          </div>

          <div className="flex gap-3 mt-6">
            <Btn full onClick={handlePublish} size="lg" disabled={saving}>
              {saving ? 'Publishing...' : '🚀 Publish Product'}
            </Btn>
            <Btn variant="outline" onClick={() => navigate('/farmer/products')} disabled={saving}>Cancel</Btn>
          </div>
        </Card>
      </div>
    </FarmerLayout>
